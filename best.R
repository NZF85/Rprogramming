best <- function(state, outcome) {
  ...
  ##looks at the particular column under the header name
  mortality_rate <- data[,header_name]
  ## removes the data that has the entry "not Available"
  ...
  ##because data is still a factor we need to 
  data[,header_name]<-as.numeric(as.character(data[,header_name]))
